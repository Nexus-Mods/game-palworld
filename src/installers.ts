...
//#region INI Config Mod
export async function testIniMod(files: string[], gameId: string): Promise<types.ISupportedResult> {
  const rightGame = gameId === GAME_ID;
  if (!rightGame) return { supported: false, requiredFiles: [] };

  const hasGamePrefix = files.some(f => {
    const lower = f.toLowerCase().replace(/\\/g, '/');
    return lower.startsWith('pal/') || lower.startsWith('engine/') || lower.startsWith('resources/') || lower.startsWith('saved/');
  });
  if (hasGamePrefix) return { supported: false, requiredFiles: [] };

  const hasIni = files.some(f => {
    const base = path.basename(f).toLowerCase();
    return base === 'engine.ini' || base === 'input.ini';
  });

  return { supported: hasIni, requiredFiles: [] };
}

export async function installIniMod(api: types.IExtensionApi, files: string[], destinationPath: string, gameId: string): Promise<types.IInstallResult> {
  const state = api.getState();
  const discovery = selectors.discoveryByGame(state, gameId);
  const winFolder = discovery?.store === 'xbox' ? 'WinGDK' : 'Windows';

  const engineIniDest = path.join('Pal', 'Saved', 'Config', winFolder, 'Engine.ini');
  const inputIniConfigDest = path.join('Pal', 'Saved', 'Config', winFolder, 'Input.ini');
  const inputIniRootDest = 'Input.ini';
  ...
  const instructions = files.reduce((accum, iter) => {
    ...
    if (baseName === 'engine.ini') {
      accum.push({
        type: 'copy',
        source: iter,
        destination: engineIniDest,
      });
    } else if (baseName === 'input.ini') {
      accum.push({
        type: 'copy',
        source: iter,
        destination: inputIniConfigDest,
      });
      accum.push({
        type: 'copy',
        source: iter,
        destination: inputIniRootDest,
      });
    } else {
      accum.push({
        type: 'copy',
...
async function installModInModsFolder(api: types.IExtensionApi, files: string[], destinationPath: string, gameId: string, extensions: string[]): Promise<types.IInstallResult> {
  ...
  let hasEnabledTxt = files.some(f => path.basename(f).toLowerCase() === 'enabled.txt');
  ...
  if (!hasEnabledTxt && folderId) {
    instructions.push({
      type: 'generatefile',
      data: '',
      destination: path.join('Mods', folderId, 'enabled.txt'),
    });
  }
